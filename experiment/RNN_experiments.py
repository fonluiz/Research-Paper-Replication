...
def create_model(x_train, y_train, x_test, y_test):
    ...
    model = Sequential()
    model.add(SimpleRNN({{choice([64, 128, 256, 512, 1024])}}, input_shape=(int(sys.argv[2]), int(sys.argv[3])), dropout={{uniform(0, 0.3)}}, return_sequences=True, recurrent_dropout={{uniform(0,0.3)}}))
    if conditional({{choice(['two', 'three'])}}) == 'three':
        model.add(SimpleRNN({{choice([64, 128, 256, 512, 1024])}}, input_shape=(int(sys.argv[2]), int(sys.argv[3])),
                       dropout={{uniform(0, 0.3)}}, return_sequences=True, recurrent_dropout={{uniform(0, 0.3)}}))
        if conditional({{choice(['three', 'four'])}}) == 'four':
            model.add(SimpleRNN({{choice([64, 128, 256, 512, 1024])}}, input_shape=(int(sys.argv[2]), int(sys.argv[3])),
                           dropout={{uniform(0, 0.3)}}, return_sequences=True, recurrent_dropout={{uniform(0, 0.3)}}))
    model.add(SimpleRNN({{choice([64, 128, 256, 512, 1024])}}, input_shape=(int(sys.argv[2]), int(sys.argv[3])), dropout={{uniform(0, 0.3)}}, recurrent_dropout={{uniform(0, 0.3)}}))
...
if __name__ == "__main__":
    ...
    numpy.savetxt("Results/RNN/" + sys.argv[1] + "_" + str(globalvar.timestep) + ".csv", numpy.atleast_2d(measures),
                  delimiter=',', fmt='%6f', header="acc, f1, precision, recall, auc")
    numpy.savetxt("Results/RNN/" + sys.argv[1] + "_" + str(globalvar.timestep) + "_SK.csv", numpy.atleast_2d(measuresSK),
                  delimiter=',', fmt='%6f', header="acc, f1, precision, recall, auc")
    text_file = open("Results/RNN/Hyperparameters" + sys.argv[1] + "_" + str(globalvar.timestep) + ".txt", "w")
