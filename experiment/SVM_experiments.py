...
for i in range(timestepsFrom, timestepsTo):
    ...
    if __name__ == '__main__':
            ...
            rand_list = {"C": sp_randint(1, 100),
             "gamma": sp_randint(0.0001, 1000)}
            #create model

            clf = svm.SVC(1)

            randomizedSearch = RandomizedSearchCV(estimator=clf, param_distributions=rand_list, scoring=scoring, cv=10, n_jobs=-1, refit='F1', n_iter=15)
            ...
            numpy.savetxt("Results/SVM/"+log+"_"+str(num_timesteps)+".csv", numpy.atleast_2d(metrics), delimiter = ',', fmt='%6f',header = "AUC, Accuracy, F1, Precision, Recall")
